...
try:
    from app.auth import auth_manager, require_auth, require_tier, UserTier
    AUTH_ENABLED = True
except ImportError:
    AUTH_ENABLED = False
...
def main():
    ...
    if AUTH_ENABLED:
        ...
        auth_manager.render_tier_badge()
    ...
    try:
        ...
        net = NetworkUtils()
        network_ok = net.test_connection()
        
        if network_ok:
            st.sidebar.success("✅ Network: Connected")
        else:
            st.sidebar.error("❌ Network: Offline")
            st.sidebar.info("Using cached data")
    except Exception as e:
        st.sidebar.warning("⚠️ Network: Unknown")
    ...
    try:
        ...
        if model_files:
            ...
            age_days = (datetime.now().timestamp() - latest_model.stat().st_mtime) / 86400
            
            if age_days < 7:
                st.sidebar.success(f"✅ Data: {int(age_days)} days old")
            elif age_days < 30:
                st.sidebar.warning(f"⚠️ Data: {int(age_days)} days old")
            else:
                st.sidebar.error(f"❌ Data: {int(age_days)} days old")
                st.sidebar.caption("Consider refreshing data")
        else:
            st.sidebar.info("ℹ️ Data age: Unknown")
    except Exception as e:
        pass

...
# Import indication-specific models
try:
    from indication_specific_models import IndicationSpecificModelEngine
    INDICATION_MODELS_AVAILABLE = True
except ImportError:
    INDICATION_MODELS_AVAILABLE = False
    print("⚠️  Indication-specific models not available")
...
def main():
    ...
    # TRAIN INDICATION-SPECIFIC MODELS (NEW!)
    if INDICATION_MODELS_AVAILABLE:
        print("\n" + "="*50)
        print("TRAINING INDICATION-SPECIFIC MODELS")
        print("="*50)
        
        try:
            indication_engine = IndicationSpecificModelEngine(random_state=42)
            indication_metrics = indication_engine.train_indication_models(df, balance_classes=True)
            
            # Save indication models
            indication_engine.save_models(model_dir)
            
            # Plot comparison
            indication_engine.plot_indication_comparison(model_dir)
            
            print("\n✅ Indication-specific models trained successfully!")
            print("\nIndication Model Performance:")
            for indication, metrics in indication_metrics.items():
                if indication != 'general':
                    print(f"  {indication:20s}: ROC-AUC {metrics['roc_auc']:.4f}")
        
        except Exception as e:
            print(f"\n⚠️  Could not train indication models: {e}")

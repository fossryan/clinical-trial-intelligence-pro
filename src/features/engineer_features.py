...
try:
    from advanced_biotech_features import AdvancedBiotechFeatures
    ADVANCED_FEATURES_AVAILABLE = True
except ImportError:
    ADVANCED_FEATURES_AVAILABLE = False
    print("⚠️  Advanced biotech features not available")
...
class TrialFeatureEngineer:
    ...
    def create_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # 11. ADVANCED BIOTECH FEATURES (NEW!)
        if ADVANCED_FEATURES_AVAILABLE:
            print("\n🔬 Adding advanced biotech features...")
            try:
                advanced_engine = AdvancedBiotechFeatures()
                df = advanced_engine.engineer_all_advanced_features(df)
                print("✓ Advanced biotech features added successfully")
...
def main():
    ...
    # Load raw data (prefer 10k file, otherwise largest file)
    ...
    # Prefer files with "10k" in the name
    files_10k = [f for f in raw_files if '10k' in f.name]
    if files_10k:
        latest_file = max(files_10k, key=lambda p: p.stat().st_mtime)
        print(f"✓ Found 10k dataset: {latest_file.name}")
    else:
        # Otherwise use largest file by row count
        print("Checking file sizes to find largest dataset...")
        largest_file = None
        largest_size = 0
        for f in raw_files:
            try:
                ...
                print(f"  {f.name}: {size:,} rows")
                if size > largest_size:
                    largest_size = size
                    largest_file = f
            except Exception as e:
                print(f"  {f.name}: Could not read")
        latest_file = largest_file if largest_file else max(raw_files, key=lambda p: p.stat().st_mtime)
        print(f"\n✓ Using largest dataset: {latest_file.name} ({largest_size:,} rows)")
